...
- Nbsファイルを.wavなどのオーディオファイルへの書き出し(後々は.mp3などへの書き出し機能の実装も予定中)

## Nbs Rustについて

nbs-rustはminecraft noteblock songファイルフォーマットの読み込み/書き込みやオーディオのレンダリング、midiからの変換などの様々な処理を行える事を目指したrust製クレートです。
特にオーディオレンダリングについては高速性を追求した開発をしており、オーディオのリサンプリングをマルチスレッドで行ったり、その結果をキャッシュする仕組み、ミキシングにsimdアクセラレーションを行うなど、
様々な高速化を行ってきました。
また、同時にノートのオーディオを供給する`InstrumentAudioProvider`を使い、任意の仕組みでオーディオを読み込んだり、キャッシュの容量やワーカースレッド数などのパラメータを設定する機能も搭載されています。
